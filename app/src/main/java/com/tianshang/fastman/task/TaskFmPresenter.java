...
public class TaskFmPresenter extends BasePresenterFm<TaskFragment, TaskFmContract.Presenter> {
    ...
    public TaskFmContract.Presenter getContract() {
        return new TaskFmContract.Presenter() {
            ...
            public void destroyLocation() {
                TaskFmPresenter.this.destroyLocation();
            }
    ...
    private void getList() {
        ...
        list.add(new TaskListEntity("到12栋圆通快递取件", R.mipmap.img5));
        list.add(new TaskListEntity("到三食堂买大份黄焖鸡", R.mipmap.img6));
        getView().getContract().setTaskList(list);
...
//                tvResult.setText(result);
                ...
                if (location.getErrorCode() == 0) {
                    if (PreferencesUtil.getInstance().getParam("locationCity", "").equals(location.getCity())) {
                        getContract().getTaskList();
                    } else {
                        PreferencesUtil.getInstance().saveParam("locationCity", location.getCity());
                        getView().getContract().resultAddress(location.getCity());
                    }
                } else {
                    getView().getContract().resultAddress(null);
                }
            } else {
                getView().getContract().resultAddress(null);
            }
